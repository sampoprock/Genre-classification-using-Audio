...
data = pd.read_csv('F:/Major project Audio/combined1.csv')
feature = data.loc[:, 'tempo':]
labels = list(feature)
color = ['red' if l==1 else 'green' if l==2 else 'blue' if l==3 else 'orange' if l==4 else 'yellow'  for l in data['label']]
...
for random_seed in range(1, 11):
    features = array[:, 5:]
    labels = data.loc[:, 'parjaay'].dropna()
    test_size = 0.30
